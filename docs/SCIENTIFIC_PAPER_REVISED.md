...
$$r \xleftarrow{\$} \mathbb{Z}_q^*, \quad B = r \cdot H'(\text{"AURA-OPAQUE-v1/OPRF"} \| \texttt{0x00} \| \text{pwd})$$
...
**Крок 3 ($\mathcal{C}$).** OPRF-розсліплення $U = r^{-1} \cdot Z$; фіналізація: вихід OPRF обчислюється як $\text{SHA-512}(\text{"AURA-OPAQUE-v1/OPRF"} \| \texttt{0x01} \| \text{pwd} \| U)$. Рандомізований пароль:
...
$$\text{transcript\_hash} = H(\text{"AURA-OPAQUE-v1/Transcript"} \| \tau)$$

**Гібридний комбінатор (наукова новизна):**
$$\text{combined\_ikm} = dh_1 \| dh_2 \| dh_3 \| dh_4 \| ss_{\text{kem}} \quad (128 + 32 = 160 \text{ байт})$$
$$\text{salt} = \text{"AURA-OPAQUE-PQ-v1/Combiner"} \| \text{transcript\_hash}$$
...
*Примітка:* Мітки у наведених формулах скорочені для стислості. Повні доменно-розділові мітки у реалізації мають вигляд `AURA-OPAQUE-PQ-v1/SessionKey`, `AURA-OPAQUE-PQ-v1/MasterKey`, `AURA-OPAQUE-PQ-v1/ResponderMAC`, `AURA-OPAQUE-PQ-v1/InitiatorMAC`. Аналогічно, у фазі реєстрації: `AURA-OPAQUE-v1/OPRF-Key`, `AURA-OPAQUE-v1/KSF`.
...
- `AURA-OPAQUE-v1/` --- класичні операції (OPRF, конверт, функція жорсткого перетворення);
- `AURA-OPAQUE-PQ-v1/` --- гібридні операції (комбінатор, деривація ключів).
...
**Версіонування через доменні мітки.** Протокол використовує версіоновані мітки (`AURA-OPAQUE-v1/` та `AURA-OPAQUE-PQ-v1/`). Майбутні версії (наприклад, з ML-KEM-1024 або іншим KEM) можуть використовувати мітку `AURA-OPAQUE-PQ-v2/`. Транскриптне зв'язування (Підрозділ 4.4) гарантує, що обидві сторони повинні узгодити версію: будь-яка невідповідність міток призведе до незбіжності гешів транскрипту та відхилення MAC.
