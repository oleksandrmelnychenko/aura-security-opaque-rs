...
% FIGURE SLOT F3 (recommended, see docs/FIGURE_PLAN.md):
% Add a protocol-flow diagram for registration and KE1--KE2--KE3 before the
% tables if space permits. It should show only stable protocol fields and the
% transcript-binding points; avoid implementation-specific crate names here.
...
% FIGURE SLOT F5 (recommended, see docs/FIGURE_PLAN.md):
% Add a hybrid-combiner/key-schedule block diagram after Eq.~\eqref{eq:hybrid-prk}.
% The figure should show 4DH outputs and the ML-KEM shared secret entering
% HKDF-Extract under transcript salt, then expanding to K_s, K_m, and MAC keys.
...
% FIGURE SLOT F8 (recommended after server benchmark refresh, see docs/FIGURE_PLAN.md):
% Add a runtime profile figure once the benchmark tables are regenerated on the
% Linux/Xeon server. It should compare Argon2id, ML-KEM full round, 4DH, KE2,
