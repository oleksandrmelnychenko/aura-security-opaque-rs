...
property of augmented PAKE: in an honest execution, the password is
neither transmitted to nor reconstructed by the record-holding remote
node, which operates on blinded OPRF input and stores only the
registration record. The construction adds a post-quantum component for
the ``harvest now, decrypt later'' threat model. The protocol consists
of two phases:
