...
On the Linux/Xeon benchmark platform used for this paper, end-to-end authentication takes 836.11~ms, a full ML-KEM-768 round costs 492.30~$\mu$s, and Argon2id is the main latency component. The post-quantum layer adds 379.45~$\mu$s to the KSF-free key-exchange path and increases the handshake size to 2715~bytes while preserving the three-message interaction profile. Relative to classical OPAQUE, the construction adds limited computation for honest parties while moving most overhead to message expansion.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Measured empirical overheads.} The post-quantum layer
        increases key-exchange CPU cost by 379.45~$\mu$s without the
...
Figure~\ref{fig:protocol-flow} gives the full message flow and marks
where the post-quantum material and account context enter the
...
\begin{tikzpicture}[
  ...
  \fill[eclgreen!12,opacity=0.92] (O) -- (X) -- (XY) -- (Y) -- cycle;
  \fill[eclred!18,opacity=0.88] (0.72,0.72,0) -- (XY) -- (1,1,0.34)
    -- (0.72,0.72,0.34) -- cycle;
  \fill[eclred!10,opacity=0.72] (Z) -- (XZ) -- (XYZ) -- (YZ) -- cycle;

  \draw[gray!65] (O) -- (X) -- (XY) -- (Y) -- cycle;
  \draw[gray!45] (Z) -- (XZ) -- (XYZ) -- (YZ) -- cycle;
  \draw[gray!45] (O) -- (Z);
  \draw[gray!45] (X) -- (XZ);
  \draw[gray!45] (Y) -- (YZ);
  \draw[gray!45] (XY) -- (XYZ);

  \draw[->,thick,eclblue!80!black] (O) -- (1.22,0,0)
    node[anchor=west] {classical source};
  \draw[->,thick,eclgold!90!black] (O) -- (0,1.22,0)
    node[anchor=west] {ML-KEM source};
  \draw[->,thick,eclred!80!black] (O) -- (0,0,1.22)
    node[anchor=south] {extractor term};

  \node[align=center,text=eclgreen!45!black,fill=white,fill opacity=0.78,
    text opacity=1,inner sep=1.5pt] at (0.38,0.24,0.14)
    {at least one source\\remains pseudorandom};
  \node[align=center,text=eclred!65!black,font=\small\bfseries]
    at (0.88,0.90,0.35) {both sources\\weakened};
  \node[align=center,text=eclred!70!black,font=\small]
    at (0.56,0.48,1.06)
    {$\mathrm{Adv}_{\mathrm{HKDF}}^{\mathrm{ext}}$};
  \node[anchor=north,text=eclblue!80!black] at (0.82,-0.05,0)
    {$\mathrm{IKM}_{\mathrm{cl}}$};
\end{tikzpicture}
}
\caption{Geometric reading of the hybrid-combiner condition used in
Eq.~\eqref{eq:hybrid-advantage-bound}. The diagram restates the adopted
interpretation: final-key recovery is modeled as requiring simultaneous
weakening of the classical and post-quantum sources, up to the extractor
term.}
...
% FIGURE SLOT F8 (optional runtime profile, see docs/FIGURE_PLAN.md):
% The benchmark tables below use the refreshed Linux/Xeon measurements.
% A log-scale runtime profile can compare Argon2id, ML-KEM full round,
% 4DH, KE2, KE3, and end-to-end authentication on the same platform.
...
Table~\ref{tab:protocol-bench} reports the corresponding protocol-level
Criterion workloads.
...
Registration & Remote-node response creation & 71.42~$\mu$s & [70.98; 71.84] & OPRF evaluation \\
...
phase measurements and the complete authentication scenario are separate
Criterion workloads with different state-initialization and batching
boundaries. They should therefore be interpreted as scenario-specific
runtime profiles rather than as phase-by-phase summands. Remote-node KE2
generation remains a sub-millisecond operation, whereas Argon2id
...
platform, end-to-end authentication takes 836.11~ms, Argon2id is the
