...
This paper specifies Hybrid PQ-OPAQUE, an OPAQUE-based construction in which ML-KEM-768 (FIPS~203) is incorporated into the authenticated 4DH exchange path without adding a protocol round. The session key is derived by HKDF-Extract from four Ristretto255 Diffie--Hellman values and the ML-KEM-768 shared secret under a transcript-dependent salt. The specification fixes the wire format, account-context binding through $\acct$, deterministic derivation of the OPRF key from the root secret $\sigoprf$, and transcript-level incorporation of the post-quantum components.

The analysis combines ProVerif~2.05, a surrogate 4DH model in Tamarin~Prover~1.10.0, and a Rust implementation validated by deterministic, integration, and property-oriented tests. The formal results are interpreted within explicit boundaries: they do not constitute a machine-checked proof of the implementation, and the offline-resistance claim is limited to compromise of the server database without compromise of $\sigoprf$.

On Apple~M1~Pro, end-to-end authentication takes 481.43~ms, a full ML-KEM-768 round costs 122.01~$\mu$s, and Argon2id is the main latency component. The post-quantum layer adds 159.3~$\mu$s to the key-exchange path without the KSF and increases the handshake size to 2715~bytes without changing the number of rounds. Relative to classical OPAQUE, the construction adds limited computation for honest parties while moving most overhead to message expansion.
\end{abstract}
...
This reliance on classical hardness assumptions is a concern in the presence of quantum adversaries. Shor's algorithm~\cite{shor1994} applies to cryptosystems based on the discrete logarithm problem, including elliptic-curve key exchange and PAKE constructions built on related assumptions~\cite{bernstein2017}. Earlier estimates suggested that breaking Ristretto255/Curve25519 would require a cryptographically relevant quantum computer with approximately 2{,}330 logical qubits~\cite{roetteler2017}. More recent estimates for 256-bit elliptic-curve cryptography over \codeid{secp256k1} report realizations of Shor's algorithm for ECDLP-256 requiring no more than 1200 logical qubits and 90 million Toffoli operations, or no more than 1450 logical qubits and 70 million Toffoli operations, respectively~\cite{babbush2026}. Although these figures are not direct estimates for Ristretto255, they support the broader view that migration planning for 256-bit elliptic-curve systems should not wait for practical attacks. The store-now-decrypt-later strategy~\cite{mosca2018}, in which an adversary archives encrypted traffic for future decryption, adds to that motivation for systems protecting long-lived confidential data.
...
This paper studies a narrower engineering question: whether OPAQUE can be reinforced with ML-KEM-768 while preserving its three-message authentication flow. The construction combines four Ristretto255 Diffie--Hellman values with an ML-KEM-768 shared secret, binds an explicit account context $\acct$ and wire-format versioning into the transcript, and evaluates the result using symbolic analysis and an executable Rust implementation. The contribution is not a general theory of hybrid PAKE composition; it is an implementation-level augmented-PAKE design with explicit syntax, stated assumptions, and a deliberately limited claim set.
...
design rationale rather than as an independently proved property.
...
handling, and FFI errors are checked by tests & Tests do not establish
...
\begin{description}
  ...
  \item[R4] (Interaction preservation): preservation of three messages and
        1.5 rounds of interaction;
  \item[R5] (Implementability): a reproducible Rust
        implementation suitable for further analysis and engineering.
\end{description}
...
\subsubsection{Generation of KE1 from Client to Server}
...
\subsubsection{Generation of KE2 from Server to Client}
...
\subsubsection{Generation of KE3 from Client to Server}

\begin{enumerate}
  \item OPRF finalization, opening of the envelope, and recovery of the
        server public key, client static key, and client public key;
  \item Mirror 4DH:
        $dh_1 = sk_C \cdot PK_S$,
...
\]
is treated as pseudorandom under the standard extractor interpretation
of HKDF-Extract. This gives the intended AND-style reading of the key
schedule: explicit recovery of the real session key from the transcript
requires compromise of \emph{both} sources. The statement is not a
...
\textbf{Long-term server-key exposure.} Compromise of $sk_S$ permits
impersonation of the server in future sessions and makes the
static-server DH terms computable from a recorded transcript, including
...
client static secret, the server ephemeral scalar, the
...
certify the Rust/FFI implementation line by line. ProVerif is used in
...
that exercises serialization, state handling, and FFI behavior
...
multiple client completions. This result is not a computational proof
...
\textbf{Client pipeline} & Initiator logic and client-side protocol phases & 900 & cryptographic core \\
\textbf{Server pipeline} & Responder logic and session completion & 907 & cryptographic core \\
\textbf{Interlanguage layer} & FFI interface and external protocol invocation & 2026 & client and server pipelines \\
\midrule
...
FFI documentation drift into describing different protocols. Table
...
long, and unsupported-version messages; FFI length accessors expose the
...
FFI lifecycle wrappers & Regression tests check state consumption,
rejection after invalid transitions, and safe release paths across the
C ABI boundary. \\
Public FFI error contract & Agent and relay FFI modules plus the public
FFI documentation & FFI lifecycle tests verify that unsupported protocol
...
labels, version handling, and rejection behavior; and the FFI
...
cost for honest parties rises additively and remains fixed-round,
whereas within the present threat model explicit compromise of the
final key material is not reducible to weakening the classical
contribution alone. It is interpreted as requiring compromise of both
the classical and post-quantum layers.
...
\midrule
Round complexity & 3 messages, 1.5 rounds & 3 messages, 1.5 rounds & Protocol interaction is unchanged \\
...
FFI contract. These layers point to the same protocol description, but
...
FFI documentation. This makes it possible to compare the written
...
of the Rust implementation and its tests. For post-quantum migration
...
archived surrogate 4DH model, and the Rust implementation exercises the
...
The implementation and verification artifact is maintained with the
...
property-oriented tests, FFI bindings, and benchmark harnesses. The
...
and the executable code. The repository also retains supporting
documentation for the FFI layer and reproducibility workflow, so that
