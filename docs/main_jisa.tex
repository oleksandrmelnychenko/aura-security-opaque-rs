...
blinding scalar, $k$ is the server's secret key, and $\mathrm{ctx}$ is
the domain-separation context for the OPRF invocation.
...
\begin{tikzpicture}[
  ...
  \node[lane] (client) at (0,0) {Client $\mathcal{C}$};
  \node[lane] (server) at (11,0) {Server $\mathcal{S}$};
...
\caption{Hybrid PQ-OPAQUE message flow. ML-KEM public and ciphertext
...
\begin{tikzpicture}[
  ...
  \node[block,kdf] (combined) at (6.1,-1.05)
    {$\mathrm{combined\_ikm}$\\$\mathrm{IKM}_{\mathrm{cl}}\|ss_{\mathrm{kem}}$};
