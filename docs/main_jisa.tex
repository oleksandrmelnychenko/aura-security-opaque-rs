...
\begin{enumerate}
  ...
  \item \textbf{Layered formal and implementation support.} Reduction sketches, two
        ...
        representation, and executable conformance tests are aligned
        to the same construction and the same threat-model boundaries.
...
Define the account-context hash as a length-prefixed binding of the
wire-format version and the account identifier:
\begin{equation}
  \eta =
  H\!\bigl(
    \Label{ctx} \,\|\, \Len(v) \,\|\, v \,\|\,
...
\end{equation}
Here $\Len(\cdot)$ denotes the canonical byte-length encoding used by
the transcript serializer. The length prefixes prevent ambiguity between
the version field and account bytes, while the inclusion of $v$ prevents
two wire-format versions from sharing the same account context.
...
\toprule
\# & Property & ProVerif & Tamarin (surrogate 4DH model) & Executable tests \\
\midrule
...
The implementation and verification artifact is maintained as an
accompanying source package. For review, release, or archival purposes it
should be supplied as a source bundle, versioned source snapshot, or
DOI-backed archive corresponding to the revision described in this paper. The Rust
workspace contains the protocol crates, deterministic and property-oriented
tests, external API bindings, and benchmark harnesses. The formal directory
contains the ProVerif and Tamarin models together with archived
verification logs. For the artifact state described in this paper, the
Rust workspace test run reports 157 passed tests and one intentionally
ignored slow regression, while the archived symbolic evidence records 5/5
ProVerif queries and 8/8 Tamarin lemmas for the documented models.
